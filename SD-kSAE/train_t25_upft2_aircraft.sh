#!/bin/bash -l
#SBATCH --job-name=train_t25_upft2_aircraft
...
mkdir -p $WORK/revelio/SD-kSAE/features/aircraft/SDv1-5/step25_upft2
...
python train_ksae.py \
  --model_name $WORK/revelio/SD-kSAE/aircraft/SDv1-5/timestep_25/up_blocks_2 \
  --feature_dir $WORK/revelio/SD-kSAE/features/aircraft/SDv1-5/step25_upft2 \
  --module_name up_blocks_2 \
  --dataset_name dpdl-benchmark/fgvc_aircraft \
  --d_in 640 \
  --expansion_factor 64 \
  --k 32 \
  --lr 0.0004 \
