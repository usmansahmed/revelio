...
def ddim_step_contiguous(pipe, sample, noise_pred, timestep):
    """
    Deterministic DDIM update from literal training timestep t to t-1.

    This avoids treating `25` as the 25th element of an inference schedule.
    Revelio's extractor passes time_step directly to scheduler.add_noise(), so
    diffusion_timestep=25 is the literal scheduler timestep 25.
    """
    ...
    if timestep > 0:
        alpha_prev = alphas[timestep - 1]
    else:
        # Final deterministic step from t=0 to the clean latent.
        alpha_prev = torch.tensor(1.0, device=sample.device, dtype=torch.float32)

    pred_x0 = (
        sample_f - torch.sqrt(1.0 - alpha_t) * noise_f
    ) / torch.sqrt(alpha_t)

    prev_sample = (
        torch.sqrt(alpha_prev) * pred_x0
        + torch.sqrt(1.0 - alpha_prev) * noise_f
    )
...
def denoise_multistep(
    ...
    """
    Denoise start_timestep -> ... -> 0.

    The SAE hook is active ONLY for the first UNet call at start_timestep.
    All later UNet calls are completely unmodified.
    """
    ...
    for timestep in range(int(start_timestep), -1, -1):
        ...
        if timestep == int(start_timestep) and intervention_hook is not None:
            if hook_module is None:
                raise ValueError("hook_module is required when intervention_hook is provided.")
            handle = hook_module.register_forward_hook(intervention_hook)
        ...
        if timestep == int(start_timestep) and intervention_hook is not None:
            hook_info = dict(intervention_hook.info)
...
def main():
    ...
    summary = {
        ...
        "denoising_mode": "contiguous_ddim_eta0",
        "intervention_only_at_start_timestep": True,
    ...
    for source_index in source_indices:
        ...
        for condition in conditions:
            if condition == "baseline":
                final_latent, hook_info = denoise_multistep(
                    pipe=pipe,
                    noisy_latent=noisy_latent,
                    prompt_embeds=prompt_embeds,
                    start_timestep=cfg["diffusion_timestep"],
                )
            else:
            ...
            recon = decode_latent(pipe, final_latent)[0].float().cpu()
        ...
        # Quantify and visualize insertion effects relative to the same multi-step baseline.
        ...
        target_mae = float(target_diff.mean().item())
        target_mse = float(((target_insertion - baseline) ** 2).mean().item())
        control_mae = float(control_diff.mean().item())
        control_mse = float(((control_insertion - baseline) ** 2).mean().item())
        ...
        target_delta_l2 = image_summary["conditions"]["target_insertion"].get("delta_l2", 0.0)
        control_delta_l2 = image_summary["conditions"]["control_insertion"].get("delta_l2", 0.0)

        if target_delta_l2 > 0:
            image_summary["target_mae_per_delta_l2"] = target_mae / target_delta_l2
            image_summary["target_mse_per_delta_l2_sq"] = target_mse / (target_delta_l2 ** 2)

        if control_delta_l2 > 0:
            image_summary["control_mae_per_delta_l2"] = control_mae / control_delta_l2
            image_summary["control_mse_per_delta_l2_sq"] = control_mse / (control_delta_l2 ** 2)

        save_tensor_image(target_diff, item_dir / "target_insertion_diff.png")
        save_tensor_image(control_diff, item_dir / "control_insertion_diff.png")
        save_tensor_image(
            torch.clamp(target_diff * cfg.get("difference_map_scale", 10.0), 0, 1),
            item_dir / "target_insertion_diff_scaled.png",
        )
        save_tensor_image(
            torch.clamp(control_diff * cfg.get("difference_map_scale", 10.0), 0, 1),
            item_dir / "control_insertion_diff_scaled.png",
        )

        print(
            f"  target insertion: MAE={target_mae:.8f}, MSE={target_mse:.8e} | "
            f"control insertion: MAE={control_mae:.8f}, MSE={control_mse:.8e}"
        )
